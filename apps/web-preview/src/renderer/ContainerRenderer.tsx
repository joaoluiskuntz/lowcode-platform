...
function getColumnClasses(node: IrContainerNode): string {
  if (node.containerType !== "column") return "";
...
export function ContainerRenderer({ node }: ContainerRendererProps) {
  ...
  if (node.containerType === "row") className.unshift("row", "g-3");
  if (node.containerType === "column") className.unshift(getColumnClasses(node));
